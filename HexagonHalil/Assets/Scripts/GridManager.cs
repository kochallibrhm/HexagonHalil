...
public class GridManager : GridClass {
	...
	private List<int> ExplodeHexagons(List<Hexagon> list) {
		...
		foreach (int i in missingColumns) {
			for (int j=0; j<gameGrid[i].Count; ++j) {
				...
				positionY = (HEX_DISTANCE_VERTICAL * j * 2) + GRID_VERTICAL_OFFSET + (OnStepper(i) ? HEX_DISTANCE_VERTICAL : 0);
				gameGrid[i][j].SetY(j);
				gameGrid[i][j].SetX(i);
	...
	private IEnumerator ProduceHexagons(List<int> columns, List<List<Color>> colorSeed = null) {
		...
		foreach (int i in columns) {
			...
			positionY = (HEX_DISTANCE_VERTICAL * gameGrid[i].Count * 2)  + GRID_VERTICAL_OFFSET + (stepperStatus ? HEX_DISTANCE_VERTICAL : 0);
	...
	private float GetGridStartCoordinateX() {
		return gridWidth/2 * -HEX_DISTANCE_HORIZONTAL;
	}
